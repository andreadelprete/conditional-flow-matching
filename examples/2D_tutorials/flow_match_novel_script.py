...
VF_BATCH_SIZES = [64] # 32 or 64 both worked very well
...
# p1 = torch.distributions.MultivariateNormal(torch.zeros(dim), torch.diag(torch.Tensor([5., 0.6])))
        ...
        if(METHOD == "OPTIMAL_TRANSPORT" or METHOD=="STANDARD_CFM"):
            ...
            for k in range(int(buffer_size)):
                ...
                for i in range(batch_size):            
                    ...
                    prob = torch.exp(p0.log_prob(x0_xbar))
                    ...
                    t_vs_p.append([t[i], norm_prob[0].item()])
                data_buffer.append([t, xt, ut])
...
if(METHOD=="NOVEL"):
    ...
    t_vs_p = np.array(t_vs_p)
    plt.figure()
    plt.plot(t_vs_p[:,0], t_vs_p[:,1], ' x', alpha=0.1)
    plt.xlabel("Time")
    plt.ylabel("Normalized prob")
    plt.grid(True)
    plt.show()
else:
    plt.figure()
